...
const NavBar = () => {
  ...
  // Landing page navigation (before login)
  ...
  const getNavItems = () => {
    if (!currentUser) return landingNavItems
    if (currentUser.role === 'owner') return ownerNavItems
          ...
          <div className="mt-4 pt-4 border-t border-teal-200 dark:border-white/20">
            {currentUser ? (
              <div className="flex flex-col gap-2">
                ...
                <button 
                  onClick={handleLogout}
                  className="inline-flex items-center gap-1 rounded-md px-3 py-2 text-sm transition bg-teal-600 text-white hover:bg-teal-700 dark:bg_white/10 dark:hover:bg-white/20 dark:text-white"
                >
                  <LogOut size={14} />
                  Logout
                </button>
